...
function afficheUl() {
	...
	$resultat = requeteSQL($strSQL);
	$menu_retour = '<ul id="menu" class="menu">';
	while ($tabl_result = mysqli_fetch_array($resultat)) {
	    if ($tabl_result['frame'] == '1'){
		$menu_retour .= '<li><a href="#t'.$tabl_result['Id'].'"class="icon fa '.$tabl_result['Nom'].'_icon" id="m'.$tabl_result['Id'].'"></a></li>';
	    }
	    else {
	        $menu_retour .= '<li><a href="#t1" onclick= window.open("'.$tabl_result['Lien'].'") class="icon fa '.$tabl_result['Nom'].'_icon" id="m'.$tabl_result['Id'].'"></a></li>';
	    }
	}
	return $menu_retour;
}
...
function afficheIframe() {
	...
	$resultat = requeteSQL($strSQL);
	while ($tabl_result = mysqli_fetch_array($resultat)) {
	    $menu_retour .= '<div class="page" id="p'.$tabl_result['Id'].'"><iframe name="m'.$tabl_result['Id'].'" class="iframe lazyload" loading="lazy" frameborder="0" data-src="'.$tabl_result['Lien'].'"></iframe></div>';
	}
	return $menu_retour;
}
...
function affichePageDeGarde() {
    ...
    $menu_retour .= '<script type="text/javascript"> location.href = "#t'.$tabl_result['Id'].'"</script>';
    return $menu_retour;
}
