...
function afficheUl() {
	...
	while ($tabl_result = $resultat->fetchArray()) {
	    if ($tabl_result['frame'] == '1'){
	        ...
	        $menu_retour .= '<li id="'.$tabl_result['Id'].'" data-id="'.$tabl_result['Id'].'" data-nom="'.$tabl_result['Nom'].'" data-lien="'.$tabl_result['Lien'].'" data-frame="'.$tabl_result['frame'].'"><a onclick= window.open(\''.$tabl_result['Lien'].'\') class="icon '.$tabl_result['Nom'].'_icon" id="m'.$tabl_result['Id'].'"></a></li>'."\n";
	    }
	}
	return $menu_retour;
}
